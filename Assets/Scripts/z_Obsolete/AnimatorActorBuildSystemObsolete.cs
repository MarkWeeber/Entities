...
public partial struct AnimatorActorBuildSystemObsolete : ISystem
{
    ...
    public partial struct OrganizeAnimatorActorsJob : IJobEntity
    {
            ...
            )
        {
            ...
            for (int i = 0; i < AnimatorBase.Length; i++)
            {
                ...
                {
                    var x = AnimatorBase[i].AnimatorControllerEntity;
                    animatorFound = true;
                    break;
                }
            }
            if (animatorFound)
            {
                ...
                if (ParametersLookup.TryGetBuffer(Entity.Null, out DynamicBuffer<AnimatorParameter> animatorParameters))
                {
                ...
                if (AnimatorLayersLookup.TryGetBuffer(Entity.Null, out DynamicBuffer<AnimatorLayersData> animatorLayers))
                {
